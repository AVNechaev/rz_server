...
@set vm_args=%releases_dir%\%release_version%\vm_win.args
...
@%erlsrv% add %service_name% -c "%description%" -name %node_name%@%host_ip% -w "%node_root%" -m "%start_erl%" -args "%args%" -stopaction "init:stop()."
@goto :EOF
...
:console
@start "%node_name% console" %werl% -boot "%node_boot_script%" -config "%sys_config%" -args_file "%vm_args%" -name %node_name%@%host_ip%
@goto :EOF
...
start "%node_name% attach" %werl% -boot "%clean_boot_script%" -remsh %node_name%@%host_ip% -name %node_name%_console@%host_ip% -setcookie %erlang_cookie%
@goto :EOF
